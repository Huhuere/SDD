...
for fold_i in range(fold):
    # os.uname() is not available on Windows; use platform.node() as a cross-platform fallback
    try:
        host_name = os.uname().nodename  # POSIX systems
    except AttributeError:
        import platform
        host_name = platform.node() or platform.system()
    print(f"I am process {os.getpid()}, running on {host_name}: starting ({time.asctime()})")
    ...
    # Some very new or very old GPUs may not be supported by the current torch build.
    # If unsupported, fall back to CPU to avoid silent early termination.
    try:
        if torch.cuda.is_available():
            cap = torch.cuda.get_device_capability()
            supported_caps = {(5,0),(5,2),(6,0),(6,1),(7,0),(7,5),(8,0),(8,6),(8,9),(9,0)}
            if cap not in supported_caps:
                print(f"[WARN] GPU capability {cap} not in supported set {supported_caps}. Falling back to CPU.")
                os.environ['CUDA_VISIBLE_DEVICES'] = ''
        else:
            print('[INFO] CUDA not available, using CPU.')
    except Exception as e:
        print(f"[WARN] Capability check failed: {e}. Proceeding.")
