...
def train(audio_model, train_loader, test_loader, args):
    ...
    while epoch < args.n_epochs + 1:
        ...
        if len(train_loader) == 0:
            print('[ERROR] train_loader 为空。请检查 train_*.scp 是否包含文件名，及音频路径是否正确。提前终止。')
            return

        for i, (audio_input, labels) in enumerate(tqdm.tqdm(train_loader)):
            if i == 0:
                print('[DEBUG] first batch preparing ...')
...
def train_daic(audio_model, train_loader, test_loader, args, dict_my):
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (audio_input, labels,_) in enumerate(tqdm.tqdm(train_loader)):
            ...
            if global_step == 0:
                print('[WARN] 没有任何 batch 被处理 (global_step 仍为 0)。可能原因：')
                print('  1) DataLoader 读取不到样本（列表文件为空或路径不对）')
                print('  2) 首个 batch 在解码时静默异常被吞（建议开启 num_workers=0 并打印）')
                print('  3) 进程被外部中断')
                return
