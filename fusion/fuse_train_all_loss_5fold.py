...
# 是否按受试者(编号)聚合后再评估 (多数投票)，True 则替换默认片段级指标
AGG_PERSON = True
AGG_PERSON_STRATEGY = 'vote'  # 'vote' 或 'prob_mean'
if CLEAR_OLD_LOG:
    try:
...
for fold_i in range(fold):
    ...
    for epoch in range(options.N_epochs):
        ...
        if epoch % options.N_eval_epoch == 0:
            ...
            # 默认片段级指标
            accuracy_sample = metrics.accuracy_score(matrix_label, matrix_pred) if len(matrix_label)>0 else 0
            precision_sample = metrics.precision_score(matrix_label, matrix_pred, average='macro', zero_division=0) if len(matrix_label)>0 else 0
            recall_sample = metrics.recall_score(matrix_label, matrix_pred, average='macro', zero_division=0) if len(matrix_label)>0 else 0
            f1_sample = metrics.f1_score(matrix_label, matrix_pred, average='macro', zero_division=0) if len(matrix_label)>0 else 0
            sensitivity_sample = TP / (TP + FN) if (TP + FN) > 0 else 0.0
            specificity_sample = TN / (TN + FP) if (TN + FP) > 0 else 0.0

            # 受试者级多数投票/概率平均聚合
            use_person_metrics = False
            if AGG_PERSON and len(person_name) > 0:
                person_true = []
                person_pred_vote = []
                person_prob_mean = []
                for probs_list, preds_list, labels_list in zip(person_roc, person_matrix, person_lable):
                    if len(labels_list) == 0:
                        continue
                    # 二分类标签聚合（多数）：等价 round(mean)
                    true_label = int(round(np.mean(labels_list)))
                    person_true.append(true_label)
                    # 概率均值（如果存的是整行 softmax，取正类列）
                    probs_arr = np.array(probs_list)
                    if probs_arr.ndim == 2 and probs_arr.shape[1] == options.num_classes:
                        pos_mean = probs_arr[:,1].mean()
                    else:
                        pos_mean = probs_arr.mean() if probs_arr.size>0 else 0.0
                    person_prob_mean.append(pos_mean)
                    # 多数投票
                    vote_label = int(round(np.mean(preds_list))) if len(preds_list)>0 else 0
                    person_pred_vote.append(vote_label)

                if len(person_true) > 0:
                    if AGG_PERSON_STRATEGY == 'prob_mean':
                        person_pred_final = [1 if p >= 0.5 else 0 for p in person_prob_mean]
                    else:  # vote
                        person_pred_final = person_pred_vote
                    try:
                        accuracy = metrics.accuracy_score(person_true, person_pred_final)
                        precision = metrics.precision_score(person_true, person_pred_final, average='macro', zero_division=0)
                        recall = metrics.recall_score(person_true, person_pred_final, average='macro', zero_division=0)
                        f1_score = metrics.f1_score(person_true, person_pred_final, average='macro', zero_division=0)
                        # 计算敏感度/特异度（需要混淆矩阵）
                        cm_p = confusion_matrix(person_true, person_pred_final, labels=[1,0])
                        TPp = cm_p[0,0] if cm_p.size else 0
                        FNp = cm_p[0,1] if cm_p.size>1 else 0
                        FPp = cm_p[1,0] if cm_p.shape[0]>1 else 0
                        TNp = cm_p[1,1] if cm_p.shape[0]>1 and cm_p.shape[1]>1 else 0
                        sensitivity = TPp/(TPp+FNp) if (TPp+FNp)>0 else 0
                        specificity = TNp/(TNp+FPp) if (TNp+FPp)>0 else 0
                        use_person_metrics = True
            ...
            # AUC：若使用受试者级，则重算 person 级正类概率；否则用片段级
            try:
                if use_person_metrics and len(person_prob_mean)>0:
                    auc = metrics.roc_auc_score(person_true, person_prob_mean)
                else:
            ...
            print("epoch %i, loss_tr=%f err_tr=%f loss_te=%f err_te=%f fold=%f best_f1=%f (%s-level)" % (epoch, loss_tot,err_tot,loss_tot_dev,err_tot_dev,fold_i,best_acc,'person' if AGG_PERSON else 'sample'))
            print('=' * 89)
            with open(options.output_folder+"/res.res", "a") as res_file:
                res_file.write("epoch %i, level=%s loss_tr=%f err_tr=%f loss_te=%f err_te=%f fold=%f best_f1=%f acc=%f pre=%f rec=%f sen=%f spe=%f auc=%f\n" % (epoch, 'person' if use_person_metrics else 'sample', loss_tot,err_tot,loss_tot_dev,err_tot_dev,fold_i,best_acc,accuracy,precision,recall,sensitivity,specificity,auc))

            # 仍使用宏平均 F1 作为模型选择标准
            if f1_score > best_acc:  # 现在 f1_score 可能是受试者级或片段级，视 AGG_PERSON 而定
                best_acc = f1_score
    ...
    if best_ckpt_path is not None and os.path.isfile(test_graph_file):
        try:
            ...
            with torch.no_grad():
                ...
                roc_label = np.array([])
                roc_pred = np.array([])
                person_matrix_t = {p: [] for p in person_name}
                person_probs_t = {p: [] for p in person_name}
                person_label_t = {p: [] for p in person_name}
                for data in test_loader_final:
                    ...
                    # 如果启用受试者聚合，收集
                    if AGG_PERSON and hasattr(data, 'name_my'):
                        names_batch = data.name_my
                        for i_num, name_get in enumerate(names_batch):
                            # 复用前面解析逻辑的简化版
                            n = name_get
                            while isinstance(n, (list, tuple)) and len(n) > 0:
                            ...
                            if pid in person_matrix_t:
                                person_matrix_t[pid].append(int(pred.cpu().numpy()[i_num]))
                                person_label_t[pid].append(int(lab.cpu().numpy()[i_num]))
                                person_probs_t[pid].append(pred_soft.cpu().numpy()[i_num])
            ...
            if AGG_PERSON and len(person_name)>0:
                ...
                person_pred_vote_t = []
                person_prob_mean_t = []
                for pid in person_name:
                    labs_p = person_label_t.get(pid, [])
                    preds_p = person_matrix_t.get(pid, [])
                    probs_p = person_probs_t.get(pid, [])
                    if len(labs_p)==0:
                        continue
                    true_lab = int(round(np.mean(labs_p)))
                    person_true_t.append(true_lab)
                    # 多数投票
                    vote_lab = int(round(np.mean(preds_p))) if len(preds_p)>0 else 0
                    person_pred_vote_t.append(vote_lab)
                    probs_arr = np.array(probs_p)
                    if probs_arr.ndim==2 and probs_arr.shape[1]==options.num_classes:
                        person_prob_mean_t.append(probs_arr[:,1].mean())
                    else:
                        person_prob_mean_t.append(probs_arr.mean() if probs_arr.size>0 else 0.0)
                if len(person_true_t)>0:
                    test_acc_person = metrics.accuracy_score(person_true_t, person_pred_vote_t)
                    test_f1_person = metrics.f1_score(person_true_t, person_pred_vote_t, average='macro')
                    try:
                        auc_test_person = metrics.roc_auc_score(person_true_t, person_prob_mean_t)
                    except:
                        auc_test_person = 0
                    print(f'[Fold {fold_i}] TEST(Person) -> Acc={test_acc_person:.4f} F1={test_f1_person:.4f} AUC={auc_test_person:.4f}')
                    with open(options.output_folder+"/res.res", "a") as res_file:
                        res_file.write(f"[Fold {fold_i}] TEST_PERSON Acc={test_acc_person:.4f} F1={test_f1_person:.4f} AUC={auc_test_person:.4f}\n")
