...
class GAT_my_loss_decoupled(torch.nn.Module):
    def __init__(self, my_model_option):
        ...
        # recovery layers (inside __init__ scope)
        # encoder_same 输出维度 = num_att_features//2 (e.g. 128), diff 分支 = num_node_features (e.g. 64)
        # 拼接 -> 192
        recon_in_dim = (self.my_model_option.num_att_features // 2) + self.my_model_option.num_node_features
        # 原先重建 attsinc 只输出 128 (num_node_features*2) 导致与其它 4 个 256 维拼接后总长度 1152 与原始拼接 1280 不匹配。
        # 调整为输出 256 维以对齐原始输入 (5*256)。
        self.recon_attsinc = Linear(recon_in_dim, self.my_model_option.num_att_features)  # 256
        self.recon_emotion = Linear(recon_in_dim, self.my_model_option.num_emotion_features) # 256
        self.recon_pause = Linear(recon_in_dim, self.my_model_option.num_att_features)       # 256
        self.recon_enegy = Linear(recon_in_dim, self.my_model_option.num_att_features)       # 256
        self.recon_js = Linear(recon_in_dim, self.my_model_option.num_att_features)          # 256

        # classification heads
        self.lin_attsinc_class = torch.nn.Sequential(
            torch.nn.Linear(self.my_model_option.num_att_features, 128),
        ...
        # projection for shared (same) representations to node feature dim for GAT
        self.proj_same = Linear(self.my_model_option.num_att_features // 2, self.my_model_option.num_node_features)
    ...
    def forward(self, x, edge_index, edge_index_d, batch):
        ...
        # Project same features to node dimension (64) for GAT and cosine losses
        x_attsinc_same = self.proj_same(x_attsinc_same)
        x_emotion_same = self.proj_same(x_emotion_same)
        x_pause_same = self.proj_same(x_pause_same)
        x_enegy_same = self.proj_same(x_enegy_same)
        x_js_same = self.proj_same(x_js_same)

        # Stack nodes: shape (B,5,64)
        x_all_same = torch.stack([x_attsinc_same, x_emotion_same, x_pause_same, x_enegy_same, x_js_same], dim=1)
        x_all_diff = torch.stack([x_attsinc_diff, x_emotion_diff, x_pause_diff, x_enegy_diff, x_js_diff], dim=1)
